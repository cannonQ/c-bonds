{
  // =====================================================================
  // Covenant Bullet Open Order
  //
  // Borrower-side loan request for a CovenantBulletBond. The borrower
  ...
  // the principal to the borrower script. The only other path is
  // cancel, authorized by borrower co-spend.
  //
  // This order originates ONE product: a bullet (installment == 0,
  // paymentsRemaining == 0) with a MANDATORY maintenance covenant
  // (thresholdBps != 0). The escrow/K machinery stays — covenant cranks
  // consume one bounty per checkpoint, so escrow == bounty * K exactly,
  // K = (term - 1) / period.
  //
  ...
  //         periodBlocks, paymentsRemaining (must be 0),
  //         ignored-at-order, maintenanceThresholdBps (must be nonzero),
  //         escrowBalance]
  ...
  // Rev-4's minCoupon slot is gone with the installment floor: a bullet
  // has no coupon to floor, so the pack is nine wide, not ten.
  ...
  // revealed in var 0, element 1 == the resolved pool NFT (the covenant
  // is mandatory here, so the pool NFT is always present), element 2 ==
  ...
  // defaults before the shared conformance check runs. The card's
  // minCoupon field (c7(8)) is read by the instalment order only — a
  // bullet has no coupon, so it is ignored here.
      ...
      card.R8[Coll[Coll[Byte]]].isDefined &&
      {
        ...
        c7(0) >= 0L && c7(1) >= 0L && c7(2) >= 0L && c7(3) >= 0L &&
        {
          ...
          conformsWith((Coll(bounty, grace, carve, haircut, thrMin, thrMax,
            minOrd, minPer, 1L), (poolNft, collatId)))
        }
      }
    }
  }
  ...
  val matchOk =
    ...
    (if (hasPin)
      ...
      conformsWith((Coll(CRANK_BOUNTY, GRACE_BLOCKS, LIQ_CARVEOUT, HAIRCUT_KEEP,
        10000L, 30000L, MIN_ORDER_VALUE, MIN_PERIOD, 0L),
