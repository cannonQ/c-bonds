{
  ...
  // Instalment Bond
  //
  // A live loan repaid on a schedule: collateral (value + tokens) locked
  // under the terms in R4-R9 until repaid, liquidated, or accelerated.
  // The coupon is the ONLY schedule advance — there is no crank arm.
  // The covenant is optional per template (thresholdBps == 0 turns it
  // off), so a schedule-only bond and a schedule-plus-covenant bond are
  // the same script. Two kinds of arm:
  //   - signatureless: liquidate, coupon, accelerate, missedAccel.
  ...
  //
  // Paths:
  //   Repay        borrower co-spend, any height, sched(2) <= 1. This
  //                bond reaches repay only after every interior coupon —
  //                early repayment of the remaining schedule is
  //                deliberately impossible. Collateral tokens must
  //                return to the borrower (collateralToBorrower).
  ...
  //   Coupon       signatureless checkpoint advance, valid only when
  //                OUTPUTS(1) pays the installment to the lender in the
  //                same transaction. No upper height bound: past the
  //                grace deadline a late coupon and missedAccel are BOTH
  //                valid and the first confirmation wins.
  ...
  //   MissedAccel  signatureless payment default: a checkpoint that
  //                passed graceBlocks ago unpaid. Liquidation shape; no
  //                health test (a payment default is not a collateral
  //                default).
  ...
  //   - liquidate requires HEIGHT >= maturity; coupon, accelerate and
  //     missedAccel all require HEIGHT < maturity
  ...
  //   - the three application sites (coupon, cure, accelerate) each pass
  //     a structurally distinct argument, so no two sites can merge into
  //     one shared eager node. Accelerate prices SELF; coupon and cure
  //     both price exitBox and are separated by the coupon's post-state
  //     escrow term (escrow - crankBounty against cure's escrow), kept
  ...
  // sched(2) <= 1 is the no-early-repayment rule and is load-bearing
  // here: only the final payment may take this path.
  val repayOk =
    sched(2) <= 1L &&
    toLender &&
  ...
  // Successor invariant shared by coupon, top-up and cure: the script
  ...
  // unhealthy NOW. The verdict argument is the exact pre-state on SELF,
  // which is what keeps this call site structurally distinct from the
  // coupon and cure sites — both of those price exitBox (see COMPILER
  // CONSTRAINT).
  val accelerateOk =
    nextCheck < 0L &&
  ...
  // call-site count at three), and no stored flag: "overdue" derives
  ...
  sigmaProp(
    liquidateOk || couponOk || accelerateOk || missedAccelOk ||
